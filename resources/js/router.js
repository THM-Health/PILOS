...
export const routes = [
  ...
  },
  {
    ...
    meta: {
      disabled: () =>
        !useSettingsStore().getSetting("user.password_change_allowed") ||
...

  {
    ...
    meta: {
      requiresAuth: true,
      accessPermitted: (userPermissions) =>
        Promise.resolve(userPermissions.can("view", "AdminPolicy")),
    },
    children: [
      {
        path: "",
...
export async function beforeEachRoute(router, to, from, next) {
  ...
  const recordsPermissions = await Promise.all(
    to.matched.map((record) =>
      record.meta.accessPermitted
        ? record.meta.accessPermitted(userPermissions, to.params, to.query)
  ...
  if (
    to.matched.some((record) => record.meta.guestsOnly) &&
    auth.isAuthenticated
  ) {
