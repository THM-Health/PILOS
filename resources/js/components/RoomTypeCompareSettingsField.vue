...
<template>
  <div class="field grid grid-cols-12 gap-4 mx-0 mb-6">
    ...
    <div class="col-span-3 flex justify-center items-center" :aria-label="currentEnforced? $t('rooms.change_type.current_setting_enforced'): $t('rooms.change_type.current_setting')">
      <Tag v-if="type === 'switch' && currentValue" class="h-6 w-6" rounded severity="primary" data-test="current-enabled">
        <span class="fas fa-check" aria-hidden="true"></span>
        <span class="sr-only">{{ $t('app.enabled') }}</span>
      </Tag>
      <Tag v-if="type === 'switch' && !currentValue" class="h-6 w-6" rounded severity="secondary" data-test="current-disabled">
    ...
    <div class="col-span-2 flex justify-center items-center" data-test="current-enforced">
    ...
    <div class="col-span-3 flex justify-center items-center" :aria-label="newEnforced? $t('rooms.change_type.resulting_setting_enforced'): $t('rooms.change_type.resulting_setting')">
      <Tag v-if="type === 'switch' && newValue" class="h-6 w-6" rounded severity="primary" data-test="new-enabled">
        <span class="fas fa-check" aria-hidden="true"></span>
        <span class="sr-only">{{ $t('app.enabled') }}</span>
      </Tag>
      <Tag v-if="type === 'switch' && !newValue" class="h-6 w-6" rounded severity="secondary" data-test="new-disabled">
        <span class="fa-solid fa-xmark" aria-hidden="true"></span>
        <span class="sr-only">{{ $t('app.disabled') }}</span>
      </Tag>
    ...
    <div class="col-span-2 flex justify-center items-center" data-test="new-enforced">
