...
class RoomFileService
{
    ...
    protected function checkFileExists(): bool
    {
        ...
        if (!Storage::exists($this->file->path)) {
            try {
                Log::error('Room file {file} not found', ['file' => $this->file->id,'filename'=>$this->file->filename ]);
                $this->file->delete();
    ...
    {
        Log::info('Download room file {file}', ['file' => $this->file->id,'filename'=>$this->file->filename ]);
    ...
    {
        Log::info('Create download url for room file {file}', ['file' => $this->file->id,'filename'=>$this->file->filename ]);
