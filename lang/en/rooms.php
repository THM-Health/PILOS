<?php

declare(strict_types=1);

return [
    'access_code' => 'Access code',
    'auth_throttled' => '[0,1] Too many attempts. Please try again in 1 second. |[2,*] Too many attempts. Please try again in :count seconds.',
    'bbb_error_message' => [
        'guestDeniedAccess' => 'Failed to join meeting: You have been denied access to this meeting based on the meeting\'s guest policy.',
        'maxParticipantsReached' => 'Failed to join meeting: The number of participants allowed for this meeting has been reached.',
        'meetingForciblyEnded' => 'Failed to join meeting: The meeting is no longer running.',
    ],
    'become_member' => 'Become member',
    'change_participant_name' => 'Change name',
    'change_type' => [
        'changing_settings' => 'The following changes will be made to the room settings',
        'current_setting' => 'Current setting',
        'current_setting_enforced' => 'Current setting. Enforced',
        'reset_to_default' => 'Reset to the default settings of the room type',
        'resulting_setting' => 'Resulting setting',
        'resulting_setting_enforced' => 'Result setting. Enforced',
        'title' => 'Change room type',
    ],
    'connection_error' => [
        'detached' => 'Connection to ongoing video conference lost, recovery failed.',
        'reconnecting' => 'Connection to ongoing video conference lost, trying to reconnect.',
    ],
    'continue_as_guest' => 'Continue as guest',
    'create' => [
        'ok' => 'Create',
        'title' => 'Create room',
    ],
    'description' => [
        'color' => [
            'black' => 'Black',
            'blue' => 'Blue',
            'green' => 'Green',
            'red' => 'Red',
        ],
        'external_link_warning' => [
            'description' => 'You are beeing redirected to the following URL :link',
            'title' => 'External link',
        ],
        'heading1' => 'Heading 1',
        'heading2' => 'Heading 2',
        'heading3' => 'Heading 3',
        'highlight' => [
            'green' => 'Green',
            'red' => 'Red',
            'yellow' => 'Yellow',
        ],
        'missing' => 'No description available',
        'modals' => [
            'image' => [
                'alt' => 'Alternative text',
                'edit' => 'Edit image',
                'invalid_src' => 'The URL must start with https://',
                'new' => 'Add new image',
                'src' => 'URL',
                'width' => 'Width',
                'width_description' => 'Width in pixels (px) or percent (%)',
            ],
            'link' => [
                'edit' => 'Edit link',
                'invalid_url' => 'The URL must start with http://, https:// or mailto://',
                'new' => 'Add new link',
                'url' => 'URL',
            ],
            'source_code' => [
                'title' => 'Edit source code',
            ],
        ],
        'paragraph' => 'Paragraph',
        'save' => 'Save',
        'title' => 'Description',
        'tooltips' => [
            'bold' => 'Bold',
            'center' => 'Align center',
            'clear' => 'Clear formatting',
            'color' => 'Text color',
            'delete' => 'Delete content',
            'highlight' => 'Highlight text',
            'image' => 'Image',
            'italic' => 'Italic',
            'left' => 'Align left',
            'link' => 'Link',
            'list' => 'List',
            'numbered_list' => 'Numbered list',
            'quote' => 'Quote',
            'redo' => 'Redo',
            'right' => 'Align right',
            'source_code' => 'Source code',
            'strikethrough' => 'Strikethrough',
            'text_type' => 'Text type',
            'underline' => 'Underline',
            'undo' => 'Undo',
        ],
    ],
    'end_membership' => [
        'button' => 'End membership',
        'message' => 'You will lose the role associated with your membership. To become a member again, you may need to ask the room owner. If the room has an access code, you will need to know it to continue to have access to the room.',
        'no' => 'No, keep membership',
        'title' => 'Are you sure you want to end the membership?',
        'yes' => 'Yes, end membership',
    ],
    'favorites' => [
        'add' => 'Add to favorites',
        'add_for' => 'Add to favorites: :room',
        'remove' => 'Remove from favorites',
        'remove_for' => 'Remove from favorites: :room',
    ],
    'feature_disabled_roomtype' => 'This feature is disabled for this room type. Try selecting a different room type or contact the administrator.',
    'feature_disabled_system' => ':name is disabled. Please contact the administrator.',
    'files' => [
        'configure' => 'Configure file',
        'configure_aria' => 'Configure file: :filename',
        'configure_dialog_title' => 'Configure ":name"',
        'configure_system_default' => 'Configure system-wide default presentation',
        'confirm_delete' => 'Do you want to delete this file :filename?',
        'confirm_default' => 'Do you want to set ":filename" as the default presentation for this room?',
        'confirm_default_use_in_meeting' => 'This will make the file available in the next video conference.',
        'confirm_default_system_default' => 'This file will replace the system-wide default presentation as the default presentation.',
        'confirm_system_default_default' => 'Do you want to set the system-wide default presentation as the default presentation for this room?',
        'confirm_system_default_default_current_default' => 'This will replace the current default presentation for this room as the default presentation. The current default will remain as a fallback and will be used if the system-wide default presentation is not available',
        'default' => 'Default presentation',
        'default_automatic' => 'Default presentation (Automatic)',
        'default_fallback' => 'Default presentation fallback',
        'default_fallback_description' => 'This file will be used as the default presentation if no system-wide presentation is available.',
        'delete' => 'Delete file',
        'delete_aria' => 'Delete file: :filename',
        'download_allowed' => 'Download allowed',
        'download_not_allowed' => 'Download not allowed',
        'downloadable' => 'Downloadable',
        'filter' => [
            'all' => 'All files',
            'downloadable' => 'Downloadable files',
            'use_in_meeting' => 'Files available in video conference',
        ],
        'filter_aria' => 'Filter files',
        'nodata' => 'No files available',
        'reload_aria' => 'Reload files',
        'search_aria' => 'Search files',
        'select_or_drag' => 'Select a file or drag and drop it here...',
        'set_default' => 'Set as default presentation',
        'set_default_aria' => 'Set file :filename as default presentation',
        'set_system_default_default_aria' => 'Set system-wide default presentation as default presentation',
        'set_system_default_default_dialog_title' => 'Set system-wide default presentation as default presentation',
        'set_default_dialog_title' => 'Set ":name" as default presentation',
        'sort' => [
            'filename' => 'Filename',
            'uploaded_at' => 'Added',
        ],
        'sort_ascending' => 'Sort files ascending',
        'sort_by' => 'Sort files by',
        'sort_descending' => 'Sort files descending',
        'system_default' => 'System-wide default presentation',
        'system_default_description' => 'Automatically used as default when no other presentation is enabled for the next video conference',
        'system_default_fallback_description' => 'Automatically used as default because no other presentation is enabled for the next video conference',
        'terms_of_use' => [
            'accept' => 'I accept the terms of use',
            'close' => 'Close terms of use',
            'required' => 'You must accept the terms of use before downloading this file.',
            'title' => 'Terms of Use',
        ],
        'title' => 'Files',
        'upload' => 'Upload files',
        'uploaded' => 'File \':name\' uploaded',
        'always_available_in_meeting' => 'Always available in next video conference',
        'available_in_next_meeting' => 'Available in next video conference',
        'available_in_next_meeting_automatic' => 'Available in next video conference (Automatic)',
        'not_available_in_next_meeting' => 'Not available in next video conference',
        'view' => 'View file',
        'view_aria' => 'View file: :filename',
    ],
    'first_and_lastname' => 'First- and last name',
    'flash' => [
        'access_code_invalid' => 'The access code is invalid.',
        'auth_token_invalid' => 'The authentication token is invalid.',
        'default_presentation_not_set' => 'The system-wide default presentation is not set.',
        'file_forbidden' => 'The access to the requested file was forbidden.',
        'file_gone' => 'The file has been deleted in the meantime.',
        'no_new_room' => 'You do not have the necessary permission to create a new room.',
        'personalized_link_gone' => 'The personalised room link has been deleted in the meantime.',
        'personalized_link_invalid' => 'The personalised room link is no longer valid.',
        'recording_forbidden' => 'The access to the requested recording has been denied.',
        'recording_gone' => 'The recording has been deleted in the meantime.',
        'start_forbidden' => 'The room could not be started by you.',
    ],
    'index' => [
        'filter' => 'Filter',
        'filter_by_room_type' => 'Filter by room type',
        'no_favorites' => 'No rooms marked as favorites',
        'no_rooms_selected' => 'No rooms selected',
        'only_favorites' => 'Only show favorites',
        'reset_filter' => 'Reset filter',
        'room_component' => [
            'details' => 'Details',
            'last_ran_till' => 'Last run until :date',
            'never_started' => 'Never started before',
            'open' => 'Open',
            'owner' => 'Owner',
            'room_type' => 'Room type',
            'running_since' => 'Running since :date',
            'short_description' => 'Short description',
            'show_details' => 'Show room details',
            'show_details_for' => 'Show details for room: :room',
        ],
        'search_aria' => 'Search rooms',
        'show_all' => 'All rooms',
        'show_own' => 'Own rooms',
        'show_public' => 'Public rooms',
        'show_shared' => 'Shared rooms',
        'sort_by' => 'Sort rooms by',
        'sorting' => [
            'alpha' => 'Alphabetical',
            'last_started' => 'Last started',
            'room_type' => 'By room type',
            'select_sorting' => '-- Select sorting --',
        ],
    ],
    'invalid_personalized_link' => 'This personalised room link is invalid.',
    'invitation' => [
        'close_aria' => 'Close share popover',
        'code' => 'Access code',
        'copied_code' => 'Copied access code to clipboard',
        'copied_message' => 'Copied invitation message to clipboard',
        'copied_url' => 'Copied room link to clipboard',
        'copy_code' => 'Copy access code',
        'copy_message' => 'Copy invitation message',
        'copy_url' => 'Copy room link',
        'link' => 'Link',
        'room' => 'Join ":roomname" with :platform',
        'share' => 'Share',
        'title' => 'Access for participants',
    ],
    'join' => 'Join',
    'join_room' => 'Join room',
    'login' => 'Login',
    'meeting_history' => [
        'reload_aria' => 'Reload history',
        'sort_ascending' => 'Sort history ascending',
        'sort_by' => 'Sort history by',
        'sort_descending' => 'Sort history descending',
        'title' => 'History',
    ],
    'members' => [
        'add_single_user' => 'Add single user',
        'add_user' => 'Add user',
        'bulk_edit_user' => '{1} Edit 1 member |[2,*] Edit :count members',
        'bulk_import_users' => 'Add several users',
        'bulk_remove_user' => '{1} Remove 1 member |[2,*] Remove :count members',
        'edit_user' => 'Edit member',
        'edit_user_aria' => 'Edit member :firstname :lastname',
        'filter' => [
            'all' => 'All',
            'co_owner_role' => 'Only Co-owners',
            'moderator_role' => 'Only moderators',
            'participant_role' => 'Only participants',
        ],
        'filter_aria' => 'Filter members',
        'image' => 'Picture',
        'modals' => [
            'add' => [
                'add' => 'Add',
                'no_options' => 'Enter the name or email address of a user.',
                'no_options_email_only' => 'Enter the email address of a user.',
                'no_result' => 'Oops! No user was found for this query.',
                'placeholder' => 'Name or email',
                'placeholder_email_only' => 'Email',
                'searching' => 'Searching...',
                'select_role' => 'Please select a role',
                'select_user' => 'Please select the user you would like to add',
                'too_many_results' => 'Too many users were found. Please enter a more precise search term.',
            ],
            'bulk_import' => [
                'can_import_users' => '{1} User that can be added |[2,*] Users that can be added',
                'cannot_import_users' => '{1} User that can not be added |[2,*] Users that can not be added',
                'copied_invalid_users' => '{1} Copied invalid user |[2,*] Copied invalid users',
                'copy_and_close' => '{1} Copy user that could not be added and close |[2,*] Copy users that could not be added and close',
                'could_not_import_users' => '{1} User that could not be added |[2,*] Users that could not be added',
                'import_importable_button' => '{1} Add valid user |[2,*] Add valid users',
                'import_importable_question' => '{1} Do you want to continue to add the user that can be added? |[2,*] Do you want to continue to add the users that can be added?',
                'imported_users' => '{1} User that was successfully added |[2,*] Users that were successfully added',
                'label' => 'Email list',
                'list_description' => 'Every line represents a separate email address (maximum 1000)',
                'list_placeholder' => 'john.doe@domain.com'."\n"
                    .'jane.doe@domain.com',
            ],
            'edit' => [
                'title' => 'Edit :firstname :lastname',
                'title_bulk' => '{1} Edit 1 user |[2,*] Edit :count users',
            ],
            'remove' => [
                'confirm' => 'Do you want to remove :firstname :lastname from this room?',
                'confirm_bulk' => '{1} Do you want to remove 1 member from this room? |[2,*] Do you want to remove :count members from this room?',
                'title' => 'Remove member from this room',
                'title_bulk' => '{1} Remove 1 member from this room |[2,*] Remove :count members from this room',
            ],
        ],
        'nodata' => 'No members available',
        'reload_aria' => 'Reload members',
        'remove_user' => 'Remove member',
        'remove_user_aria' => 'Remove member :firstname :lastname',
        'search_aria' => 'Search members',
        'select_all_aria' => 'Select all members on this page',
        'select_aria' => 'Select member :firstname :lastname',
        'sort_ascending' => 'Sort members ascending',
        'sort_by' => 'Sort members by',
        'sort_descending' => 'Sort members descending',
        'title' => 'Members',
    ],
    'modals' => [
        'delete' => [
            'confirm' => 'Should the room ":name" be deleted?',
            'title' => 'Delete room',
        ],
        'transfer_ownership' => [
            'new_owner' => 'New owner',
            'new_role' => 'New role of the current owner',
            'no_role' => 'No role',
            'select_user' => 'Please select the new owner',
            'title' => 'Transfer ownership',
            'transfer' => 'Transfer',
            'warning' => 'Caution: This may result in the current owner losing access to this room',
        ],
    ],
    'name' => 'Room name',
    'name_in_video_conference' => 'Name in video conference: ',
    'no_rooms_available' => 'No rooms available',
    'no_rooms_found' => 'No rooms found',
    'not_running' => 'This room is not started yet.',
    'notification' => [
        'body' => 'The room was started at :time',
        'browser_support' => 'Your browser does not support notification.',
        'denied' => 'The browser denied notifications.',
        'disable' => 'Deactivate notification when starting the room',
        'disabled' => 'You will no longer be notified by your browser when the room is started.',
        'enable' => 'Notify on room start',
        'enabled' => 'You will be notified by your browser when the room starts. Do not close this window/tab.',
    ],
    'only_used_by_authenticated_users' => 'This room can only be used by authenticated users.',
    'personalized_links' => [
        'add' => 'Add personalized room link',
        'confirm_delete' => 'Do you really want to delete the personalized room link for :firstname :lastname?',
        'copy' => 'Copy personalized room link to clipboard',
        'copy_aria' => 'Copy personalized room link for :firstname :lastname to clipboard',
        'delete' => 'Delete personalized room link',
        'delete_aria' => 'Delete personalized room link for :firstname :lastname',
        'edit' => 'Edit personalized room link',
        'edit_aria' => 'Edit personalized room link for :firstname :lastname',
        'expires' => 'Expiry date',
        'expires_at' => 'Expires at :date',
        'filter' => [
            'all' => 'All',
            'moderator_role' => 'Only moderators',
            'participant_role' => 'Only participants',
        ],
        'filter_aria' => 'Filter personalized room links',
        'last_usage' => 'Last used',
        'last_used_at' => 'Last used at :date',
        'last_used_never' => 'Never used',
        'nodata' => 'No personalized room links available!',
        'reload_aria' => 'Reload personalized room links',
        'room_link_copied' => 'The personalized room link for :firstname :lastname was copied to your clipboard.',
        'search_aria' => 'Search personalized room links',
        'sort_ascending' => 'Sort personalized room links ascending',
        'sort_by' => 'Sort personalized room links by',
        'sort_descending' => 'Sort personalized room links descending',
        'title' => 'Personalized room links',
    ],
    'placeholder_name' => 'John Doe',
    'recording_accept' => 'I consent to the recording.',
    'recording_attendance_accept' => 'I consent to the attendance logging.',
    'recording_attendance_info' => 'The attendance in this room is logged.',
    'recording_hint' => 'If you don\'t want your voice recorded, make sure your microphone is muted. Use the chat to participate instead.',
    'recording_info' => 'This room may be recorded.',
    'recording_video_accept' => '(Optional) My video image may be recorded.',
    'recordings' => [
        'access' => 'Access',
        'access_types' => [
            'everyone' => 'Everyone (like room)',
            'moderator' => 'Member: Moderator',
            'owner' => 'Member: Co-owner',
            'participant' => 'Member: Participant',
        ],
        'actions' => 'Actions',
        'available_formats' => 'Available formats',
        'delete_recording' => 'Delete recording',
        'delete_recording_aria' => 'Delete recording: :description',
        'description' => 'Description',
        'download' => 'Download raw recording files',
        'download_aria' => 'Download raw recording files: :description',
        'edit_recording' => 'Edit recording',
        'edit_recording_aria' => 'Edit recording: :description',
        'end' => 'End',
        'filename' => 'Recording',
        'filter' => [
            'all' => 'All',
            'everyone_access' => 'Access: Everyone (like room)',
            'moderator_access' => 'Access: Moderator',
            'owner_access' => 'Access: Co-owner',
            'participant_access' => 'Access: Participant',
        ],
        'filter_aria' => 'Filter recordings',
        'format_types' => [
            'notes' => 'Notes',
            'podcast' => 'Audio-Podcast',
            'presentation' => 'Presentation',
            'screenshare' => 'Screenshare',
            'video' => 'Video',
        ],
        'formats' => 'Formats',
        'modals' => [
            'delete' => [
                'confirm' => 'Should the recording be irrevocably deleted?',
                'title' => 'Delete recording',
            ],
            'edit' => [
                'title' => 'Edit recording',
            ],
        ],
        'nodata' => 'No recordings available',
        'reload_aria' => 'Reload recordings',
        'retention_period' => [
            'days' => 'The recordings are stored for :days days.',
            'title' => 'Retention period',
            'unlimited' => 'The recordings are stored indefinitely.',
        ],
        'search_aria' => 'Search recordings',
        'sort' => [
            'description' => 'Description',
            'start' => 'Start',
        ],
        'sort_ascending' => 'Sort recordings ascending',
        'sort_by' => 'Sort recordings by',
        'sort_descending' => 'Sort recordings descending',
        'start' => 'Start',
        'title' => 'Recordings',
        'view_recording' => 'View recording',
        'view_recording_aria' => 'View recording: :description',
    ],
    'reload_aria' => 'Reload room',
    'remember_participant_name' => 'Remember name for next time',
    'request_participant_name_change' => 'Please change your name to join the room.',
    'require_access_code' => 'An access code is required to join this room',
    'role' => 'Role',
    'roles' => [
        'co_owner' => 'Co-owner',
        'guest' => 'Guest',
        'moderator' => 'Moderator',
        'no_role' => 'No role',
        'participant' => 'Participant',
    ],
    'room_limit' => 'Room limit: :has/:max',
    'room_type_invalid_alert' => 'The usage of the room type :roomTypeName is only permitted for special user groups. If you are the owner of this room, please change the room type so that the room can be started again.',
    'room_types' => [
        'all' => 'All room types',
        'features' => [
            'disabled' => ':name disabled',
            'enabled' => ':name enabled',
            'streaming' => 'Streaming',
            'title' => 'Features',
        ],
        'loading_error' => 'An error occurred during loading of the room types.',
        'reload' => 'Reload room types',
    ],
    'search' => 'Search by room name or owner...',
    'settings' => [
        'advanced' => [
            'title' => 'Advanced',
            'visibility' => [
                'private' => 'Private',
                'public' => 'Public',
                'title' => 'Visibility',
            ],
        ],
        'expert_mode' => [
            'activate' => 'Activate expert mode',
            'deactivate' => 'Deactivate expert mode',
            'warning' => [
                'activate' => 'When activating the expert mode new default settings of the room type will no longer be applied.',
                'deactivate' => 'When deactivating the expert mode all settings except the general settings will be reset to the default settings of the room type.',
            ],
        ],
        'general' => [
            'access_by_guests' => 'Access by guests',
            'access_code_enforced' => 'The room type enforces the existence of an access code',
            'access_code_prohibited' => 'The room type enforces the absence of an access code',
            'allow' => 'Allow',
            'allow_guests' => 'Allow guests',
            'delete_access_code' => 'Remove access code',
            'enforced_setting' => 'Enforced setting',
            'generate_access_code' => 'Create new access code',
            'has_access_code' => 'Access code',
            'short_description' => 'Public short description',
            'title' => 'General',
            'type' => 'Type',
            'unprotected_placeholder' => '-- unprotected --',
        ],
        'none_placeholder' => '-- none --',
        'participants' => [
            'allow_membership' => 'Allow new members',
            'default_role' => [
                'only_logged_in' => '(only for authenticated users)',
                'title' => 'Default role',
            ],
            'title' => 'Participants',
        ],
        'recordings' => [
            'auto_start_recording' => 'Automatic recording start',
            'record_attendance' => 'Log attendance of participants',
            'record_video_conference' => 'Record video conference',
            'title' => 'Recordings',
        ],
        'restrictions' => [
            'lock_settings_disable_cam' => 'Disable webcam',
            'lock_settings_disable_mic' => 'Disable microphone',
            'lock_settings_disable_note' => 'Disable editing of notes',
            'lock_settings_disable_private_chat' => 'Disable private chat',
            'lock_settings_disable_public_chat' => 'Disable public chat',
            'lock_settings_hide_user_list' => 'Hide list of participants',
            'title' => 'Restrict participant rights in video conferences',
            'webcams_only_for_moderator' => 'Only moderators can see the webcam',
        ],
        'title' => 'Settings',
        'unsaved_changes' => 'Caution! Room settings have been changed but not yet saved.',
        'video_conference' => [
            'everyone_can_start' => 'Everyone can start the meeting',
            'lobby' => [
                'alert' => 'Default role is set to moderator: Waiting room is active only for guests and already existing members with participant role!',
                'only_for_guests_enabled' => 'Enabled only for guests',
                'title' => 'Waiting room',
            ],
            'mute_on_start' => 'Mute microphone on join',
            'title' => 'Video conference',
            'welcome_message' => 'Welcome message',
        ],
    ],
    'start' => 'Start',
    'start_room' => 'Start room',
    'streaming' => [
        'config' => [
            'button' => 'Configure streaming',
            'default_pause_image' => 'Default pause image',
            'enabled' => 'Enabled',
            'pause_image' => 'Pause image',
            'pause_image_alt' => 'Pause image',
            'pause_image_resolution' => 'Resolution: 1920x1080px',
            'title' => 'Streaming configuration',
            'url' => 'RTMP(S) URL',
        ],
        'failed' => 'Failed',
        'fps' => ':fps FPS',
        'no_running_meeting' => 'No running meeting',
        'not_enabled_for_running_meeting' => 'Streaming is not enabled for the current meeting',
        'pause' => 'Pause',
        'paused' => 'Paused',
        'pausing' => 'Pausing',
        'queued' => 'Queued',
        'reload_aria' => 'Reload streaming status',
        'resume' => 'Resume',
        'resuming' => 'Resuming',
        'running' => 'Running',
        'start' => 'Start',
        'starting' => 'Starting',
        'stop' => 'Stop',
        'stopped' => 'Stopped',
        'stopping' => 'Stopping',
        'title' => 'Streaming',
    ],
    'streaming_accept' => 'I consent to the streaming.',
    'streaming_hint' => 'If you prefer not to have your voice or video streamed, please keep your microphone muted and your camera off.',
    'streaming_info' => 'This room may be streamed.',
    'try_again' => 'Try again',
];
