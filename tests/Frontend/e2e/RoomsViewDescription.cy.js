...
describe('Rooms view description', function () {
  ...
  it('view with different permissions', function () {
    ...
    cy.get('[data-test="room-description-viewer"]').should('be.visible').within(() => {
  ...
  it('edit description', function () {
    ...
    // Cancel editing
    cy.get('[data-test="room-description-cancel-edit-button"]').should('have.text', 'app.cancel_editing').click();
    cy.get('[data-test="room-description-cancel-edit-button"]').should('not.exist');
    cy.get('[data-test="room-description-viewer"]').should('be.visible').within(() => {
      cy.get('p').contains('Room description').should('be.visible');
    });
    cy.get('[data-test="tip-tap-editor"]').should('not.exist');
    ...
    // Save description
    const saveDescriptionRequest = interceptIndefinitely('PUT', 'api/v1/rooms/abc-def-123/description', {
      statusCode: 204
    ...
    cy.get('[data-test="room-description-cancel-edit-button"]').should('be.disabled');
    cy.get('[data-test="room-description-save-button"]').should('be.disabled').then(() => {
    ...
    cy.get('[data-test="tip-tap-menu-dropdown-item-button"]').should('have.length', 4);
    ...
    // Check that description was changed correctly
    cy.get('.tiptap').should('be.visible').within(() => {
      cy.get('h1').find('strong').contains('New test description').should('be.visible');
      cy.get('p').should('not.exist');
    });
  ...
  it('edit description errors', function () {
    ...
    cy.intercept('PUT', 'api/v1/rooms/abc-def-123/description', {
      ...
      body: {
        ...
        errors:
          {
            description: ['The Description must not be greater than 65000 characters.']
          }
      }
    }).as('saveDescriptionRequest');
    ...
    cy.contains('The Description must not be greater than 65000 characters.').should('be.visible');
    ...
    cy.checkRoomAuthErrors(() => {
      ...
      cy.get('[data-test="tip-tap-editor"]').should('be.visible');
      cy.get('[data-test="room-description-save-button"]').click();
    }, 'PUT', 'api/v1/rooms/abc-def-123/description', 'description');
  });
  ...
  it('open external link', function () {
    ...
    cy.get('[data-test="room-description-viewer-confirm-dialog"]').should('not.exist');
    cy.get('[data-test="room-description-viewer"]').should('be.visible').within(() => {
    ...
    cy.get('[data-test="room-description-viewer-confirm-dialog"]')
      ...
      .should('include.text', 'rooms.description.external_link_warning.description_{"link":"https://example.org/?foo=a&bar=b"}')
      .within(() => {
        // Cancel opening link
        cy.get('[data-test="confirm-dialog-reject-button"]').should('have.text', 'app.cancel').click();
      });
    ...
    cy.get('[data-test="room-description-viewer-confirm-dialog"]')
      ...
      .should('include.text', 'rooms.description.external_link_warning.description_{"link":"https://example.org/?foo=a&bar=b"}')
      .within(() => {
        cy.get('[data-test="confirm-dialog-accept-button"]').should('have.text', 'app.continue').click();
      });
  ...
  it('sanitize html', function () {
    ...
    cy.get('[data-test="room-description-viewer"]').should('be.visible').within(() => {
      ...
      cy.get('p')
        ...
        .should('be.visible')
        .and('have.attr', 'style', 'text-align: center; color: rgb(255, 0, 0); background-color: rgb(0, 255, 0);');
      ...
      cy.get('p')
        ...
        .should('be.visible')
        .and('have.attr', 'style', 'text-align: center; color: rgb(0, 0, 255); background-color: rgb(255, 255, 0);');
