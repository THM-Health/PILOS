...
Cypress._.times(20, () => {
  describe("Rooms view meetings", function () {
    ...
    it.only("join running meeting with personalized link errors", function () {
      cy.intercept("GET", "api/v1/currentUser", {});
      cy.interceptRoomFilesRequest();
      cy.fixture("room.json").then((room) => {
        room.data.last_meeting = {
    ...
    it("join meeting errors", function () {
      ...
      // Check if error message is shown
      cy.contains("rooms.only_used_by_authenticated_users").should(
        "be.visible",
      );
    ...
    it.only("join meeting load requirements errors", function () {
      cy.fixture("room.json").then((room) => {
        room.data.last_meeting = {
          start: "2023-08-21T08:18:28.000000Z",
          end: null,
    ...
    it.only("start meeting with personalized link errors", function () {
      cy.intercept("GET", "api/v1/currentUser", {});
    ...
    it("start meeting errors", function () {
      ...
      cy.intercept("POST", "/api/v1/rooms/abc-def-123/start*", {
        ...
        body: {
          message:
            "The room could not be started because it is already running.",
        },
      }).as("startRequest");
      ...
      cy.get('[data-test="room-join-button"]').should(
        "have.text",
        "rooms.join",
      );
      cy.get('[data-test="room-start-button"]').should("not.exist");
    ...
    it.only("start meeting load requirements errors", function () {
      cy.interceptRoomFilesRequest();
      cy.fixture("room.json").then((room) => {
        room.data.last_meeting = {
          start: "2023-08-21T08:18:28.000000Z",
