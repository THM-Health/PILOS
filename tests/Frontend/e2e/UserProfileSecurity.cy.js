...
describe('User Profile Security', function () {
  ...
  it('check view and change settings', function () {
    ...
    cy.get('[data-test="new-password-field"]')
      ...
      .within(() => {
        cy.get('#new_password').should('have.value', '').type('newSecretPassword123#');
      });
    ...
    cy.get('[data-test="new-password-confirmation-field"]')
      ...
      .within(() => {
        cy.get('#new_password_confirmation').should('have.value', '').type('newSecretPassword123#');
      });

    // Try to change password
    cy.fixture('user').then(user => {
      const saveChangesRequest = interceptIndefinitely('PUT', 'api/v1/users/1/password', {
        statusCode: 200,
        body: user
      }, 'saveChangesRequest');
      ...
      // Check loading
      cy.get('[data-test="security-tab-current-password-field"]')
        .find('#current_password').should('be.disabled');
      cy.get('#new_password').should('be.disabled');
      cy.get('#new_password_confirmation').should('be.disabled').then(() => {
    ...
    cy.get('[data-test="security-tab-current-password-field"]')
      .find('#current_password').should('have.value', '').and('not.be.disabled');
    cy.get('#new_password').should('have.value', '').and('not.be.disabled');
    cy.get('#new_password_confirmation').should('have.value', '').and('not.be.disabled');
    ...
    const logoutAllRequest = interceptIndefinitely('DELETE', 'api/v1/sessions', {
      statusCode: 204
    }, 'logoutAllRequest');
    ...
    // Check loading
    cy.get('[data-test="logout-all-sessions-button"]').should('be.disabled').then(() => {
  ...
  it('change settings errors', function () {
    ...
    cy.get('[data-test="security-tab-current-password-field"]')
      .find('#current_password').type('secretPassword123#');

    cy.get('[data-test="new-password-field"]').type('newSecretPassword123#');
    cy.get('[data-test="new-password-confirmation-field"]').type('newSecretPassword123#');
    ...
    cy.get('[data-test="security-tab-current-password-field"]')
      .should('include.text', 'The Current password field is required.');

    cy.get('[data-test="new-password-field"]')
      .should('include.text', 'The New password field is required.');

    cy.get('[data-test="new-password-confirmation-field"]')
      .should('include.text', 'The New password confirmation field is required.');
  ...
  it('load sessions error', function () {
    ...
    cy.intercept('GET', 'api/v1/sessions', { fixture: 'sessions.json' }).as('getSessionsRequest');

    cy.get('[data-test="loading-retry-button"]').should('have.text', 'app.reload').click();
