...
describe('RoomSettings', () => {
  ...
  it(
    'load settings, fill form fields, disable fields if no write permission, calculate welcome message',
    done => {
  ...
  it(
    'load settings owner, check fields disabled during loading',
    done => {
      PermissionService.setCurrentUser(exampleUser);
      moxios.stubRequest(`/api/v1/roomTypes?filter=${exampleRoom.id}`, {
        status: 200,
  ...
  it('load settings co-owner', done => {
    PermissionService.setCurrentUser(exampleUser);
    moxios.stubRequest(`/api/v1/roomTypes?filter=${exampleRoom.id}`, {
      status: 200,
      response: exampleRoomTypeResponse
  ...
  it('load settings with room manage permission', done => {
  ...
  it(
    'load and save settings with attendance logging globally disabled',
    done => {
      const store = new Vuex.Store({
        modules: {
          session: {
