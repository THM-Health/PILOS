...
import { BDropdownItemButton, BFormCheckbox, BFormRadio } from 'bootstrap-vue';
...
describe('Room Transfer Dropdown Button', () => {
  ...
  it('test transfer ownership with role', async () => {
    ...
    // find stay in room checkbox and make sure it is selected
    const stayInRoomCheckbox = view.findComponent(BFormCheckbox);
    expect(stayInRoomCheckbox.text()).toBe('rooms.modals.transfer_ownership.stay_in_room_current');
    expect(stayInRoomCheckbox.props('checked')).toBeTruthy();
  ...
  it('test transfer ownership without role', async () => {
    ...
    // find stay in room checkbox
    const stayInRoomCheckbox = view.findComponent(BFormCheckbox);
    // ToDo
    // expect(stayInRoomCheckbox.text()).toBe('rooms.modals.transfer_ownership.stay_in_room');
    expect(stayInRoomCheckbox.props('checked')).toBeTruthy();
    stayInRoomCheckbox.get('input').trigger('click');
    await view.vm.$nextTick();
    expect(stayInRoomCheckbox.props('checked')).toBeFalsy();

    // make sure role selector is hidden
    const roleSelector = modal.findAllComponents(BFormRadio);
    expect(roleSelector.length).toBe(0);
  ...
  // ToDo
  // it('test transfer ownership error', async () => {
  //
  //   view.destroy();
  // });
});
