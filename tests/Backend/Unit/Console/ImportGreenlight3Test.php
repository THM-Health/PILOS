...
class ImportGreenlight3Test extends TestCase
{
    ...
    protected function test_helper_full(array $cmdOptions, callable $expectHook, ?string $prefix = null)
    {
        ...
        if ($rollback) {
            // Check presentation files were moved back
        ...
        // Test presentations
        $this->assertFalse($storageMock->fileExists('migration/presentations/fe/en/feen6movahheegheeg0ovahche8bu3mo'));
        $this->assertEquals('1testvongpresiher.pdf', Room::find('abc-def-xyz-123')->files()->first()->filename);
        $this->assertFalse($storageMock->fileExists('migration/presentations/xi/ve/xivei7mi0cohtoecacahyaich8ohzaed'));
