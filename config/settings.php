...
return [
    ...
    */
    'defaults' => [
        ...
        'room_limit' => intval(env('DEFAULT_ROOM_LIMIT', -1)),
        //@deprecate OWN_ROOMS_PAGINATION_PAGE_SIZE
        'room_pagination_page_size' => intval(env('ROOM_PAGINATION_PAGE_SIZE', env('OWN_ROOMS_PAGINATION_PAGE_SIZE', 9))),
        'pagination_page_size' => intval(env('DEFAULT_PAGINATION_PAGE_SIZE', 15)),
        'statistics' => [
            'servers' => [
                'enabled' => env('STATISTICS_SERVERS_ENABLED', false),
                'retention_period' => intval(env('STATISTICS_SERVERS_RETENTION_PERIOD', 30)),
            ],
            'meetings' => [
                'enabled' => env('STATISTICS_MEETINGS_ENABLED', false),
                'retention_period' => intval(env('STATISTICS_MEETINGS_RETENTION_PERIOD', 30)),
            ],
        ],
        'attendance' => [
            'enabled' => env('ATTENDANCE_ENABLED', false),
            'retention_period' => intval(env('ATTENDANCE_RETENTION_PERIOD', 14)),
        ],
        'banner' => [
            'enabled' => false,
        ],
        ...
        'room_token_expiration' => intval(env('ROOM_TOKEN_EXPIRATION', -1)),
        'room_auto_delete' => [
            'enabled' => env('ROOM_AUTO_DELETE_ENABLED', false),
            'inactive_period' => intval(env('ROOM_AUTO_DELETE_INACTIVE_PERIOD', 365)),
            'never_used_period' => intval(env('ROOM_AUTO_DELETE_NEVER_USED_PERIOD', 90)),
            'deadline_period' => intval(env('ROOM_AUTO_DELETE_DEADLINE_PERIOD', 14)),
        ],
    ],
];
