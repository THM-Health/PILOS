...
Note that files uploaded during a meeting inside BigBlueButton are not scanned by PILOS.
You have to configure the BigBlueButton server to also scan files uploaded during meetings. Read the [BigBlueButton documentation](https://docs.bigbluebutton.org/administration/customize/#support-for-clamav-as-presentation-file-scanner) for more information on how to set up antivirus scanning for BigBlueButton.
...
Set the `MAX_FILE_SIZE` and `MAX_SCAN_SIZE` environment variables to a value equal to or greater than the file upload limit defined by `BBB_MAX_FILESIZE` in your `.env` file.
