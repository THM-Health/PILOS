...
| Option                    | Default Value                     | Description                                                                                                                                                                                                                                                                    |
| ------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `REFERRER_POLICY`         | `strict-origin-when-cross-origin` | Sets the `Referrer-Policy` header. See: [OWASP Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/HTTP_Headers_Cheat_Sheet.html#referrer-policy).                                                                                                                     |
| `HSTS_ENABLED`            | `false`                           | Enables HTTP Strict Transport Security (HSTS). Always disabled when `APP_ENV=local`. Enable only if the site is **always** available via HTTPS. See: [OWASP HSTS Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/HTTP_Strict_Transport_Security_Cheat_Sheet.html). |
| `HSTS_MAX_AGE`            | `31536000`                        | HSTS max-age in seconds (default: 1 year). Recommended ramp-up: 300 (5 min), 604800 (1 week), 2592000 (1 month). See: [HSTS Deployment Recommendations](https://hstspreload.org/#deployment-recommendations).                                                                  |
| `HSTS_INCLUDE_SUBDOMAINS` | `false`                           | Applies HSTS to all subdomains. Enable only if **all** subdomains are always available via HTTPS.                                                                                                                                                                              |
| `HSTS_PRELOAD`            | `false`                           | Requests inclusion in the browser preload list. **Caution:** Preloading is permanent and has serious consequences. Read carefully before enabling: [hstspreload.org](https://hstspreload.org/).                                                                                |

## More config options

- [Monitoring](./08-advanced/08-monitoring.md)
