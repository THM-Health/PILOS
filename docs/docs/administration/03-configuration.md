...
| `VERSION`         | _Version of the Docker Container, e.g. 4.0.1_ | If this variable is not set, PILOS uses the contents of the `version` file inside the container. This file is created by the official CI pipeline.<br/><br/>Set VERSION= to hide the displayed version completely, or set it to a custom value such as the major version only (VERSION=4).<br/><br/>**Please note:** Hiding or changing the displayed version does not fully conceal the actual version. An attacker may still be able to infer it from application behavior, asset files, and other implementation details. |
